...
"""
Generate a zero-dependency HTML viewer for stored FileMappings.

The UI is split into two files so the pipeline can refresh data without
touching the page:

- viewer.html      static shell — written once (or when the template changes)
- viewer_data.js   the payload — regenerated on every run from the
                   mappings/ and mermaids/ folders
...
- collect_mermaids() attaches the matching pre-rendered .mmd text from
  MERMAIDS_DIR (written by graph.graph_all_mappings) under a "_mermaid" key,
  so the UI can show the pipeline's own diagram as well as its richer
  client-side render.
- write_viewer_data() serialises the records into viewer_data.js
  (window.MAPPING_DATA = [...]).
- write_viewer() writes both the shell and the data, and is what
  workflow.run_workflow calls at the end of every pipeline run.

No server, no build step: open viewer.html in a browser. Re-running the
pipeline (or `python viewer.py`) refreshes viewer_data.js; just reload the
page. Deterministic and LLM-free, same philosophy as the persistence layer.

Usage:
    python viewer.py                      # reads persistence.MAPPINGS_DIR / graph.MERMAIDS_DIR
...
# viewer.py may live next to the pipeline modules or in a ui/ subfolder;
# make `import persistence` / `import graph` work in both layouts.
for _candidate in (_MODULE_DIR, _MODULE_DIR.parent):
...
try:
    ...
    _DEFAULT_MAPPINGS_DIR = _MODULE_DIR / "mappings"

try:
    import graph as _graph
    _DEFAULT_MERMAIDS_DIR = _graph.MERMAIDS_DIR
except ImportError:
    _DEFAULT_MERMAIDS_DIR = _MODULE_DIR / "mermaids"

_DEFAULT_OUTPUT = _MODULE_DIR / "viewer.html"
_DATA_FILE_NAME = "viewer_data.js"


# ---------------------------------------------------------------------------
# 1 · Collect stored mappings + mermaid charts
# ---------------------------------------------------------------------------
...
def collect_mermaids(
    records: list[dict[str, Any]],
    mermaids_dir: Path | None = None,
) -> list[dict[str, Any]]:
    """Attach the pre-rendered .mmd text (if any) to each mapping record.

    graph.graph_all_mappings mirrors mappings/a/b/x.mapping.json to
    mermaids/a/b/x.mmd, so the lookup is a pure path transform on the
    record's "_file". Records without a chart (e.g. unrelated pairs, which
    graph skips) simply get no "_mermaid" key — the UI falls back to its
    own client-side render.
    """
    root = Path(mermaids_dir or _DEFAULT_MERMAIDS_DIR)
    if not root.is_dir():
        logger.info("Mermaids directory %s does not exist; skipping", root)
        return records

    attached = 0
    for record in records:
        relative = record.get("_file", "")
        if not relative.endswith(".mapping.json"):
            continue
        mmd_path = root / relative.removesuffix(".mapping.json")
        mmd_path = mmd_path.with_name(mmd_path.name + ".mmd")
        if not mmd_path.is_file():
            continue
        try:
            record["_mermaid"] = mmd_path.read_text(encoding="utf-8")
            attached += 1
        except OSError as error:
            logger.warning("Skipping unreadable mermaid %s: %s", mmd_path, error)

    logger.info("Attached %d mermaid chart(s) from %s", attached, root)
    return records
...
# The template is plain HTML/CSS/JS. It loads its data from viewer_data.js
# (window.MAPPING_DATA), so this file never needs regenerating when the
# pipeline reruns — only the data file does. No str.format / f-strings, so
...
<main id="main">
  <div id="placeholder">
    <h2 id="ph-title">Select a mapping</h2>
    <p id="ph-text">Choose a file pair on the left, or use ← → to step through them.</p>
  </div>
...
<script>
const DATA = (window.MAPPING_DATA || []);
...
async function renderDiagram(rec) {
  ...
  const hasMmd = typeof rec._mermaid === "string" && rec._mermaid.trim().length > 0;
  body.innerHTML =
    '<div class="diagram-controls">' +
      '<label><input type="checkbox" id="dim-low"' + (dimLow ? " checked" : "") + "> dash edges below " + THRESHOLD + "</label>" +
      (hasMmd ? '<label><input type="checkbox" id="use-mmd"' + (useMmd ? " checked" : "") + "> use pipeline .mmd</label>" : "") +
      '<div class="legend">' + legend + "</div>" +
    "</div>" +
    '<div id="diagram-wrap"><div id="diagram">Rendering…</div></div>';
  $("#dim-low").onchange = e => { dimLow = e.target.checked; renderDiagram(rec); };
  if (hasMmd) $("#use-mmd").onchange = e => { useMmd = e.target.checked; renderDiagram(rec); };

  const seq = ++renderSeq;
  try {
    const source = (useMmd && rec._mermaid) ? rec._mermaid : buildMermaid(rec);
    const { svg } = await mermaid.render("m" + seq, source);
...
if (DATA.length) {
  ...
  $("#ph-title").textContent = "No mapping data found";
  $("#ph-text").innerHTML = "Run the pipeline, then refresh the data file with " +
    "<code>python viewer.py</code> (writes <code>viewer_data.js</code> from the " +
    "<code>mappings/</code> and <code>mermaids/</code> folders), and reload this page.";
}
</script>
</body>
</html>
...
def build_viewer_data_js(records: list[dict[str, Any]]) -> str:
    """Serialise mapping records as the viewer_data.js payload."""
    # </script> inside a JSON string would end the script block early if the
    # file were ever inlined; harmless to escape defensively either way.
    payload = json.dumps(records, indent=None).replace("</", "<\\/")
    return (
        "// Generated by viewer.py — do not edit. Rerun the pipeline to refresh.\n"
        "window.MAPPING_DATA = " + payload + ";\n"
    )


def data_path_for(output_path: Path | None = None) -> Path:
    """viewer_data.js lives next to the HTML shell it feeds."""
    target = Path(output_path or _DEFAULT_OUTPUT)
    return target.parent / _DATA_FILE_NAME
...
def write_viewer_data(
    ...
    """Collect mappings + mermaids and write viewer_data.js. Returns the path."""
    records = collect_mermaids(collect_mappings(mappings_dir), mermaids_dir)
    target = data_path_for(output_path)
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text(build_viewer_data_js(records), encoding="utf-8")
    logger.info("Wrote %d mapping(s) to %s", len(records), target)
    return target
...
def write_viewer(
    ...
    """Write the static HTML shell AND refresh its data file.

    Returns the HTML path. Called from workflow.run_workflow so the UI is
    always current after a pipeline run; also runnable standalone.
    """
    target = Path(output_path or _DEFAULT_OUTPUT)
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text(_TEMPLATE, encoding="utf-8")
    data_file = write_viewer_data(mappings_dir, mermaids_dir, output_path=target)
    logger.info("Wrote viewer shell to %s (data: %s)", target, data_file)
    return target
